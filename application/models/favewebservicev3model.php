...
class Favewebservicev3model extends CI_Model {
    ...
    public function updatedContactChange($user_id, $otpsent, $contactUpdate) {
        $this->db->where('user_id', $user_id);
        if ($this->db->update('tbl_users', array('otp_password' => $otpsent, 'contact_update' => $contactUpdate))) {
            return TRUE;
        }
    ...
    public function saveUserContact($contactArray, $user_id) {
        $this->db->where('user_id', $user_id);
        if ($this->db->update('tbl_users', $contactArray)) {
            return TRUE;
        }
        return FALSE;
    }
...
// return $this->db->affected_rows();
    ...
    public function getAllCustomerJobStatus1($posted_by_id) {
        $anyPendingIds = '';
        $anyPendingId = $this->__getAllCustomerJobStatus1($posted_by_id);
        if ($anyPendingId) {
            $anyPendingIds = ' OR tbl_jobs.id IN(' . implode(',', $anyPendingId) . ')';
        }
        $this->db->select('tbl_jobs.*, tbl_jobs_status.name as job_status, tbl_categories.price, tbl_categories.name as categories_name');
        $this->db->from('tbl_jobs');
        $this->db->join('tbl_categories', 'tbl_jobs.category_id = tbl_categories.id', 'left');
        $this->db->join('tbl_jobs_status', 'tbl_jobs.job_status_id = tbl_jobs_status.id', 'left');
        $this->db->order_by("tbl_jobs.created", "desc");
        $where = "(posted_by_id='$posted_by_id' ) AND (tbl_jobs.job_status_id='2' OR tbl_jobs.job_status_id='10' OR tbl_jobs.job_status_id='1' OR tbl_jobs.job_status_id='7' " . $anyPendingIds . ")";
        $this->db->where($where);
        $query = $this->db->get();
        if ($query->num_rows()) {
            return $query->result_array();
    ...
    private function __getAllCustomerJobStatus1($posted_by_id) {
        $response = (array) null;
        $this->db->select('tbl_applied_by.job_id');
        $this->db->from('tbl_applied_by');
        $this->db->join('tbl_jobs', 'tbl_applied_by.job_id = tbl_jobs.id', 'LEFT');
        $where = "(tbl_jobs.posted_by_id='$posted_by_id' ) AND (tbl_applied_by.job_status_id='2' OR tbl_applied_by.job_status_id='10' OR tbl_applied_by.job_status_id='1' OR tbl_applied_by.job_status_id='7')";
        $this->db->where($where);
        $this->db->group_by('tbl_applied_by.job_id');
        $query = $this->db->get();
    ...
    public function getAllCustomerJobStatusRunning($posted_by_id) {
        $this->db->select('tbl_jobs.*, tbl_jobs_status.name as job_status, tbl_categories.price, tbl_categories.name as categories_name');
        $this->db->from('tbl_jobs');
        $this->db->join('tbl_categories', 'tbl_jobs.category_id = tbl_categories.id', 'left');
        $this->db->join('tbl_jobs_status', 'tbl_jobs.job_status_id = tbl_jobs_status.id', 'left');
        $this->db->order_by("tbl_jobs.created", "desc");
        $where = "(posted_by_id='$posted_by_id' ) AND (tbl_jobs.job_status_id IN (2,10))";
        $this->db->where($where);
        $query = $this->db->get();
        if ($query->num_rows()) {
            return $query->result_array();
    ...
    public function getAllHiredUserDataBuddy($job_id) {
        ...
        $this->db->join('tbl_jobs_status', 'tbl_jobs_status.id = tbl_applied_by.job_status_id', 'left');
        $this->db->order_by("tbl_applied_by.created", "desc");
        $where = "(job_id='$job_id' ) AND (tbl_applied_by.job_status_id='8' OR tbl_applied_by.job_status_id='2' OR tbl_applied_by.job_status_id='10')";
        $this->db->where($where);
        // $this->db->where(array("job_status_id" => 8, 'job_id' => $job_id));
        $query = $this->db->get();
        if ($query->num_rows()) {
    ...
    public function getAllHiredUserDataBuddy2($job_id) {
        $appliedId = '';
        $appliedIds = $this->__getAllHiredUserDataBuddy2($job_id);
        if ($appliedIds) {
            $appliedId = 'OR tbl_applied_by.id IN(' . implode(',', $appliedIds) . ')';
        }
        $this->db->select("tbl_applied_by.applied_by_id, tbl_applied_by.job_id, tbl_users.name,tbl_users.profile_pic,tbl_users.about_me, tbl_users.contact, tbl_jobs_status.name AS job_status_now");
        $this->db->from("tbl_applied_by");
        $this->db->join('tbl_users', 'tbl_applied_by.applied_by_id = tbl_users.user_id', 'left');
        $this->db->join('tbl_jobs_status', 'tbl_jobs_status.id = tbl_applied_by.job_status_id', 'left');
        $this->db->order_by("tbl_applied_by.created", "desc");
        $where = "(job_id='$job_id' ) AND (job_id='$job_id' ) AND (tbl_applied_by.job_status_id='8' OR tbl_applied_by.job_status_id='2' OR tbl_applied_by.job_status_id='10' " . $appliedId . ") AND tbl_applied_by.job_status_id !='1' AND tbl_applied_by.job_status_id !='9'";
        $this->db->where($where);
        $query = $this->db->get();
        if ($query->num_rows()) {
            return $query->result_array();
    ...
    private function __getAllHiredUserDataBuddy2($job_id) {
        $this->db->select('job_id');
        $this->db->from('tbl_applied_by');
        $where = "(job_id='$job_id' ) AND (tbl_applied_by.job_status_id='8' OR tbl_applied_by.job_status_id='2' OR tbl_applied_by.job_status_id='10' OR tbl_applied_by.job_status_id='4' OR tbl_applied_by.job_status_id='3')";
        $this->db->where($where);
        $query = $this->db->get();
        if ($query->num_rows()) {
            $row = $query->row_array();
            return $this->__getAppliedByIds($row['job_id']);
        } else {
            return array();
        }
    }

    private function __getAppliedByIds($job_id) {
        $response = (array) null;
        $this->db->select('id');
        $this->db->from('tbl_applied_by');
    ...
    public function getAllHiredUserDataBuddy1($job_id) {

        $this->db->select("tbl_users.user_id AS buddy_user_id,tbl_applied_by.applied_by_id, tbl_applied_by.job_id, tbl_users.name,tbl_users.profile_pic,tbl_users.about_me, tbl_users.contact, tbl_jobs_status.name AS job_status_now");
        $this->db->from("tbl_applied_by");
        $this->db->join('tbl_users', 'tbl_applied_by.applied_by_id = tbl_users.user_id', 'left');
        $this->db->join('tbl_jobs_status', 'tbl_jobs_status.id = tbl_applied_by.job_status_id', 'left');
        $this->db->order_by("tbl_applied_by.created", "desc");
        $where = "(job_id='$job_id' ) AND (tbl_applied_by.job_status_id='2' OR tbl_applied_by.job_status_id='10')";
        $this->db->where($where);
        // $this->db->where(array("job_status_id" => 8, 'job_id' => $job_id));
        $query = $this->db->get();
        if ($query->num_rows()) {
    ...
    public function getAllHiredUserDataCloseBuddy($job_id) {
        $this->db->select("tbl_applied_by.applied_by_id, tbl_applied_by.job_id, tbl_users.name, tbl_users.profile_pic,tbl_users.about_me");
        $this->db->from("tbl_applied_by");
        $this->db->join('tbl_users', 'tbl_applied_by.applied_by_id = tbl_users.user_id', 'left');
        $this->db->order_by("tbl_applied_by.created", "desc");
        $where = "(job_id='$job_id' ) AND (tbl_applied_by.job_status_id IN (2,10))";
        $this->db->where($where);
        // $this->db->where(array("job_status_id" => 8, 'job_id' => $job_id));
        $query = $this->db->get();
        if ($query->num_rows()) {
    ...
    public function getReview($user_id) {
        ...
        $this->db->where('tbl_buddy_ratings.rating_to', $user_id);
        $this->db->where('tbl_buddy_ratings.feedback !=', '');
        $query = $this->db->get();
        if ($query->num_rows() > 0) {
            return $query->result_array();
        }
    ...
    public function getReviewCustomer($user_id) {
        ...
        $this->db->where('tbl_customer_ratings.feedback !=', '');
        $query = $this->db->get();
        if ($query->num_rows() > 0) {
            return $query->result_array();
        }
    ...
    public function checkWeatherWorkdoneByAnyBuddie($job_id) {
        $this->db->select('applied_by_id');
        $this->db->from('tbl_applied_by');
    ...
    public function checkUserWorkingStatus($job_id) {
        $this->db->select('applied_by_id');
        $this->db->from('tbl_applied_by');
        $where = "(job_id = $job_id) AND (job_status_id = 3 OR job_status_id = 4 OR job_status_id = 2)";
        $this->db->where($where);
        $query = $this->db->get();
        return $query->num_rows();
    }

    public function totalWorkingBuddyCount($job_id) {

        $this->db->select('applied_by_id');
        $this->db->from('tbl_applied_by');
        $where = "(job_id = $job_id) AND (job_status_id = 2 OR job_status_id = 10 OR job_status_id = 3 OR job_status_id = 4)";
        $this->db->where($where);
        $query = $this->db->get();
        return $query->num_rows();
    }
...
//    }
    ...
    public function chkHiredUserStatus($applied_by_id, $date) {
        $this->db->select('status');
        $this->db->from('tbl_chkUserHired');
        $this->db->where(array('user_id' => $applied_by_id, 'status' => 1, 'date' => $date));
    ...
    public function deleteUserFromHiredTable($applied_by_id, $job_id) {

        $this->db->delete('tbl_chkUserHired', array('job_id' => $job_id, 'user_id' => $applied_by_id));
        // $this->db->last_query();
        return TRUE;
    }

    public function deleteUsersFromHiredTable($job_id) {

        $this->db->delete('tbl_chkUserHired', array('job_id' => $job_id));
        return TRUE;
    }
    ...
    public function changejobStatusToClose($job_id) {
        ...
        $this->db->where(array('id' => $job_id));
        if ($this->db->update('tbl_jobs', array('job_status_id' => 3, 'modified' => date('Y-m-d H:i:s')))) {
            return TRUE;
        }
        return FALSE;
    }
    ...
    public function changejobStatusToCancel($job_id) {
        $this->db->where(array('id' => $job_id));
        if ($this->db->update('tbl_jobs', array('job_status_id' => 9, 'modified' => date('Y-m-d H:i:s')))) {
            return TRUE;
        }
        return FALSE;
    }
    ...
    public function getAllCustomerHistoryJob($posted_by_id) {
        ...
        $this->db->select('tbl_jobs.*,tbl_jobs_status.name as job_status, tbl_categories.name,IFNULL(`tbl_categories`.`price`,0) AS `category_price`,IFNULL((SELECT count(applied_by_id) FROM tbl_applied_by WHERE job_id = tbl_jobs.id AND job_status_id IN(2,10) GROUP BY job_id),0) AS running_count', false);
        $this->db->from('tbl_jobs');
        $this->db->join('tbl_jobs_status', 'tbl_jobs.job_status_id = tbl_jobs_status.id', 'left');
        $this->db->join('tbl_categories', 'tbl_jobs.category_id = tbl_categories.id', 'left');
...
//    }
    ...
    public function totalWorkingHour1($applied_by_id, $job_id) {
        ...
        $this->db->select("*");
        $this->db->from("tbl_applied_by");
        $where = "(job_id='$job_id' ) AND (applied_by_id='$applied_by_id' ) AND (tbl_applied_by.job_status_id IN (2,10))";
        $this->db->where($where);
        $query = $this->db->get();
        if ($query->num_rows()) {
            return $query->row_array();
    ...
    public function getAllCustomerHelpyJobnew1($posted_by_id) {
        $postIds = $this->__buddyStatusJobIds($posted_by_id);
        $this->db->select('tbl_jobs.*, tbl_jobs_status.name as job_status,tbl_categories.name,IFNULL(`tbl_categories`.`price`,0) AS `category_price`,IFNULL((SELECT count(applied_by_id) FROM tbl_applied_by WHERE job_id = tbl_jobs.id AND job_status_id IN(2,10) GROUP BY job_id),0) AS running_count', false);
        $this->db->from('tbl_jobs');
        $this->db->join('tbl_jobs_status', 'tbl_jobs.job_status_id = tbl_jobs_status.id', 'left');
        $this->db->join('tbl_categories', 'tbl_jobs.category_id = tbl_categories.id', 'left');
        $this->db->order_by("tbl_jobs.created", "desc");
        $where = "(posted_by_id='$posted_by_id' AND tbl_jobs.id IN (" . implode(',', $postIds) . "))";
        $this->db->where($where);
        $query = $this->db->get();
        if ($query->num_rows()) {
            return $query->result_array();
    ...
    private function __buddyStatusJobIds($postedBy) {
        $response = (array) null;
        $this->db->select('tbl_applied_by.job_id AS id');
        $this->db->from('tbl_jobs');
        $this->db->join('tbl_applied_by', 'tbl_jobs.id = tbl_applied_by.job_id', 'right');
        $this->db->where(array('tbl_jobs.posted_by_id' => $postedBy));
        $this->db->where('tbl_applied_by.job_status_id IN (2,10)');
        $this->db->group_by('tbl_applied_by.job_id');
        $query = $this->db->get();
        foreach ($query->result_array() as $item) {
            $response[] = $item['id'];
...
//    }
    ...
    public function getAllRunningBuddyJob1($applied_by_id, $job_id) {
        $this->db->select('tbl_applied_by.applied_by_id, tbl_applied_by.working_hour, tbl_applied_by.pau_job_status, tbl_applied_by.pause_time, tbl_applied_by.resume_time, tbl_applied_by.job_start_time, tbl_jobs.posted_by_id, tbl_users.name, tbl_users.profile_pic, tbl_users.about_me, tbl_users.contact, tbl_jobs.*, tbl_jobs_status.name as job_status, tbl_categories.price, tbl_categories.name as categories_name, tbl_applied_by.job_status_id AS job_status_id_buddy');
        $this->db->from('tbl_applied_by');
        $this->db->join('tbl_jobs_status', 'tbl_applied_by.job_status_id = tbl_jobs_status.id', 'left');
        $this->db->join('tbl_jobs', 'tbl_applied_by.job_id = tbl_jobs.id', 'left');
        $this->db->join('tbl_categories', 'tbl_jobs.category_id = tbl_categories.id', 'left');
        $this->db->join('tbl_users', 'tbl_jobs.posted_by_id = tbl_users.user_id', 'left');
        $where = "(tbl_applied_by.applied_by_id = $applied_by_id) AND tbl_applied_by.job_id = " . $job_id;
        $this->db->where($where);
        $query = $this->db->get();
        if ($query->num_rows()) {
            return $query->row_array();
...
//        ));
    ...
    public function saveHiredStatus($ratingData) {
        if ($this->db->insert('tbl_chkUserHired', $ratingData)) {
            return $this->db->insert_id();
        }
        return FALSE;
    }
...
//    }
    ...
    public function cancelSingleBuddyStatus($job_id, $applied_by_id) {

        $where = "(tbl_applied_by.job_id=$job_id) AND (tbl_applied_by.applied_by_id=$applied_by_id)";
        $this->db->where($where);
        $this->db->update('tbl_applied_by', array('job_status_id' => 9));
    }
...
//       $query = $this->db->query("SELECT tbl_users.user_id, tbl_device.device_id,((ACOS(SIN($lat * PI() / 180) * SIN(tbl_users.lat * PI() / 180) + COS($lat * PI() / 180) * COS(tbl_users.lat * PI() / 180) * COS(($lng – tbl_users.lng) * PI() / 180)) * 180 / PI()) * 60 * 1.1515) AS `distance` FROM tbl_users JOIN tbl_device ON tbl_users.user_id = tbl_device.user_id where is_buddy=1 HAVING distance < 10000 ORDER BY distance ");
        $query = $this->db->query("SELECT tbl_users.user_id, tbl_device.device_id,(((acos(sin((" . $lat . "*pi()/180)) * sin((`tbl_users`.`lat`*pi()/180))+cos((" . $lat . "*pi()/180)) * cos((`tbl_users`.`lat`*pi()/180)) * cos(((" . $lng . "- `tbl_users`.`lng`)* pi()/180))))*180/pi())*60*1.1515) as distance FROM `tbl_users` JOIN `tbl_device` ON `tbl_users`.`user_id` = `tbl_device`.`user_id` where `tbl_users`.`is_buddy` = 1 HAVING distance < 25");
//       echo $this->db->last_query();die;
    ...
    public function closeBuddyCount($job_id) {
        ...
        $this->db->where_in('job_status_id', array(3, 4));
        $query = $this->db->get();
        return $query->num_rows();
    }
...
//    
    ...
    public function getJobAllStatus($job_id) {
    ...
    public function getJobStatus($job_id, $user_id) {



        $this->db->select('job_status_id');
        $this->db->from('tbl_applied_by');
        $where = "(tbl_applied_by.job_id='$job_id' ) AND (tbl_applied_by.applied_by_id ='$user_id' ) AND (tbl_applied_by.job_status_id='3' OR tbl_applied_by.job_status_id='9')";
        $this->db->where($where);
        $query = $this->db->get();
        if ($query->num_rows()) {
            return $query->row_array();
    ...
    public function getPauseStatus($job_id, $user_id) {

        $this->db->select('job_status_id');
        $this->db->from('tbl_applied_by');
        $where = "(tbl_applied_by.job_id='$job_id' ) AND (tbl_applied_by.applied_by_id='$user_id' ) AND (tbl_applied_by.job_status_id='10')";
        $this->db->where($where);
        $query = $this->db->get();
        //$this->db->last_query();
        if ($query->num_rows()) {
    ...
    public function getResumeStatus($job_id, $user_id) {

        $this->db->select('job_status_id');
        $this->db->from('tbl_applied_by');
        $where = "(tbl_applied_by.job_id='$job_id' ) AND (tbl_applied_by.job_status_id='2') AND (tbl_applied_by.applied_by_id='$user_id')";
        $this->db->where($where);
        $query = $this->db->get();
        //$this->db->last_query();
        if ($query->num_rows()) {
...
//    }
    ...
    /*     * *************************** Update by T:307 on 05-05-2016 *************************************************************************** */

    public function getTransactions($user_id) {
        $this->db->select('fees_rate as fees_rate, fees_transactions as fees_transactions');
        $this->db->from('tbl_users');
        $this->db->where(array('user_id' => $user_id, 'fees_transactions >' => 0));
        $query = $this->db->get();
        if ($query->num_rows()) {
            return $query->row_array();
        }
    ...
    public function updateUserFeeTransaction($user_id, $fee_transaction) {
        $user_info = array('fees_transactions' => $fee_transaction);
        $this->db->where(array('user_id' => $user_id));
        $this->db->update('tbl_users', $user_info);
    }

    function get_charges($user_id) {
        $query = $this->db->query("SELECT *  FROM tbl_transaction where user_id = '$user_id'");
        if ($query->num_rows()) {
            return $query->result();
        }
        return false;
    ...
    /*     * *************************** End Updation by T:307 on 05-05-2016 *************************************************************************** */
    ...
    // function to get single country name

    public function getSingleCountry($country_id) {
    ...
    public function cancelOtherApplications($applied_by, $date, $job_id) {
        $this->db->query("UPDATE tbl_applied_by AS ab LEFT JOIN tbl_jobs AS tj ON ab.job_id = tj.id SET ab.job_status_id = 9 WHERE DATE(tj.when_buddy_needed) = '" . $date . "' AND ab.applied_by_id = '" . $applied_by . "' AND ab.job_id != '" . $job_id . "' AND ab.job_status_id = 1");
    }
    
    public function getJobStatusBuddyRunning($job_id) {
        $this->db->select('*');
        $this->db->from(DB_PREFIX.'applied_by');
        $this->db->where('(job_id = '.$job_id.' AND job_status_id = 2)');
        $query = $this->db->get();
        if($query->num_rows) {
            return true;
        }
    ...
    public function categoryPriceOnTime($category_id){
        $this->db->select('price, price2, price3, status');
        $this->db->from(DB_PREFIX . 'categories');
